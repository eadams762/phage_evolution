...
ggsave("cm_bob_map.png", plot = cm_bob_map, width = 6.5, height = 1, units = "in", dpi = 600)

# plot phage CCS4 heat map
cm_ccs4_map <- ggplot(
  cm_sum[cm_sum$seq_id == "CCS4", ],
  aes(x = factor(position), y = factor(time))
) +
  geom_tile(
    color="black",
...
ggsave("cm_ccs4_map.png", plot = cm_ccs4_map, width = 6.5, height = 1, units = "in", dpi = 600)

# plot all phages to generate complete legend
cm_all_map <- ggplot(
  cm_sum[cm_sum$seq_id != "SDS1", ],
  aes(x = factor(position), y = factor(time))
) +
  geom_tile(
    color="black",
...
cm_legend <- cowplot::get_legend(cm_all_map)
cm_legend <- ggpubr::as_ggplot(cm_legend)
ggsave("cm_legend.png", plot = cm_legend, width = 6.5, height = 0.5, units = "in", dpi = 600, scale = 2)
